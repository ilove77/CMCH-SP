...
--- 程序名稱：getDrugBranchOutStockGrantQty
--- 程序說明：取得藥品全院出庫消耗量
--- 編訂人員：蔡易志
--- 校閱人員：孫培然
--- 修訂日期：2021/07/30
CREATE FUNCTION [fn].[getDrugBranchOutStockGrantQty] (@tableName VARCHAR(20), @stockNo CHAR(04), @drugCode INT, @outStockTime1 DATETIME, @outStockTime2 DATETIME)
RETURNS INT
AS BEGIN
   DECLARE @grantQty INT = 0;
   DECLARE @tranType INT = 25; --交易類別 => 25: 消耗

   SELECT @grantQty = ISNULL(SUM(b.Qty), 0) 
